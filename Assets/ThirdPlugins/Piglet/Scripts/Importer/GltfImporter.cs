...
using CameraType = Piglet.GLTF.Schema.CameraType;
using PigletCamera = Piglet.GLTF.Schema.Camera;
using Debug = UnityEngine.Debug;
using LightType = Piglet.GLTF.Schema.LightType;
...
namespace Piglet
{
	public class GltfImporter
	{
		...
		// load light
		protected void SetupLightNodes()
		{
			foreach (var kvp in _imported.Nodes)
			{
				int nodeIndex = kvp.Key;
				GameObject gameObject = kvp.Value;
				Node node = _root.Nodes[nodeIndex];
				if(node.Extensions ==null || !node.Extensions.ContainsKey("KHR_lights_punctual"))
					continue;
				KHR_LightsPunctualNodeExtension nodeExtension = (KHR_LightsPunctualNodeExtension) node.Extensions["KHR_lights_punctual"];
				KHR_LightsPunctualExtension lights = (KHR_LightsPunctualExtension) _root.Extensions["KHR_lights_punctual"];
				PunctualLight punctualLight = lights.Lights[nodeExtension.LightId.Id];
				if (punctualLight.Type == LightType.directional) continue;
				Light light = gameObject.AddComponent<Light>();
				light.color = new Color(punctualLight.Color.R, punctualLight.Color.G, punctualLight.Color.B, punctualLight.Color.A);
				// light.intensity = (float)punctualLight.Intensity  / Mathf.PI;
				light.range = (float)punctualLight.Range;
				if (punctualLight.Type == LightType.directional)
				{
					light.intensity = (float)punctualLight.Intensity;
					light.type = UnityEngine.LightType.Directional;
				}
				if (punctualLight.Type == LightType.point)
				{
					light.intensity = (float)punctualLight.Intensity  / 500;
					light.type = UnityEngine.LightType.Point;
				}
				if (punctualLight.Type == LightType.spot)
				{
					light.intensity = (float)punctualLight.Intensity  / 5000;
					if (punctualLight.Spot != null)
					{
#if UNITY_2019_1_OR_NEWER
						light.innerSpotAngle = (float) punctualLight.Spot.InnerConeAngle * 2 / (Mathf.Deg2Rad * 0.8f);
#endif
						light.spotAngle = (float) punctualLight.Spot.OuterConeAngle * 2 / Mathf.Deg2Rad;
					}
		...
		// load camera
		protected void SetupCameraNodes()
		{
			if(_root.Cameras == null) return;
			foreach (var kvp in _imported.Nodes)
			{
				int nodeIndex = kvp.Key;
				GameObject gameObject = kvp.Value;
				...
				if(node.Camera ==null)
					continue;
				PigletCamera pigletCamera = _root.Cameras[node.Camera.Id];
				Camera camera = gameObject.AddComponent<Camera>();
				camera.backgroundColor = Camera.main.backgroundColor;
				camera.clearFlags = Camera.main.clearFlags;
				if (pigletCamera.Type == CameraType.perspective)
				{
					camera.fieldOfView = (float)pigletCamera.Perspective.YFov * Mathf.Rad2Deg;
					camera.aspect = (float)pigletCamera.Perspective.AspectRatio;
					camera.nearClipPlane = (float)pigletCamera.Perspective.ZNear;
					camera.farClipPlane = (float)pigletCamera.Perspective.ZFar;
				}
				if (pigletCamera.Type == CameraType.orthographic)
				{
					camera.orthographic  = true;
					camera.orthographicSize = (float)pigletCamera.Orthographic.XMag / 15.5f;
					camera.nearClipPlane = (float)pigletCamera.Orthographic.ZNear;
					camera.farClipPlane = (float)pigletCamera.Orthographic.ZFar;
				}
			}
		}
