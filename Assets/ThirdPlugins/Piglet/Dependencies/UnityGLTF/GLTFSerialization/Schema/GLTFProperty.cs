...
namespace Piglet.GLTF.Schema
{
	public class GLTFProperty
	{
		...
		private static KHR_lights_punctualExtensionFactory _KHR_lights_punctualExtensionFactory =
			new KHR_lights_punctualExtensionFactory();
		...
		private Dictionary<string, Extension> DeserializeExtensions(GLTFRoot root, JsonReader reader)
		{
			...
			foreach(JToken child in extensions.Children())
			{
				...
				else if (extensionName.Equals(KHR_lights_punctualExtensionFactory.EXTENSION_NAME))
				{
					extensionsCollection.Add(extensionName, _KHR_lights_punctualExtensionFactory.Deserialize(root, childAsJProperty));
				}
